...
(function () {
  ...
  async function backgroundAutoAlign() {
    ...
    try {
      ...
      // Firebase → Сервер: добавляем треки которых нет на сервере
      if (diff.onlyFirebaseCount > 0 && (diff.onlyFirebase || []).length > 0) {
        const toImport = {};
        for (const id of diff.onlyFirebase) {
          if (tracksData[id]) toImport[id] = tracksData[id];
        }
  ...
  const MIRRORED_COLS = [
    ...
    'pinnedTracks','subscriptions','subscribers','history','notifications',
...
<div style="display:flex;flex-direction:column;gap:16px">
  ...
  <div style="background:var(--md-surface-container);border-radius:16px;padding:14px;border:1px solid var(--md-outline-variant)">
    ...
    <div style="font:400 12px/18px 'Inter',sans-serif;color:var(--md-on-surface-variant);margin-bottom:10px">
      Сравни треки сервера и Firebase, найди различия и выровняй все коллекции.
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      <button class="btn-tonal" onclick="window.MusicDB.showDiff()" style="padding:8px 14px;font-size:13px">
